...
export default class LookupCommand implements Command {
    name = "manualverify";
    description =
        "Manually verify a Discord user, bypassing the university email verification process.";
    slashCommand = new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName("reason")
                .setDescription(
                    "Please input a justification for manually verifying this user."
                )
                .setRequired(true)
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<any> {
        ...
        if (!interaction.guild)
            return console.error("Guild not found in interaction.");
        ...
        let verificationUser = await MongoDb.getInstance().getVerificationUser(
            user.id
        );
        ...
        try {
            ...
            console.error(
                `Failed to fetch member or remove unverified role for user ID ${user.id}:`,
                error
            );
        }
        ...
        try {
            await this.sendManualVerificationLog(
                user,
                interaction.user,
                reason
            );
        ...
        return await interaction.editReply({
            content: `User ${userMention(user.id)} has been manually verified.`
        });
    }

    private async sendManualVerificationLog(
