...
export default class MuteCommand implements Command {
    ...
    slashCommand = new SlashCommandBuilder()
        ...
        .addUserOption((option) =>
            option
                .setName("user")
                .setDescription("Specify the user you are timing out.")
                .setRequired(true)
        )
        .addStringOption((option) =>
            option
                .setName("duration")
                .setDescription(
                    'Specify the duration for which you are timing the user out (i.e., "1 hr 20 mins")'
                )
                .setRequired(true)
        )
        .addStringOption((option) =>
            option
                .setName("reason")
                .setDescription(
                    "Please provide a justification for timing out this user."
                )
                .setRequired(true)
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<any> {
        ...
        if (!duration || duration < 60000 || duration >= 2419200000)
            return await interaction.editReply(
                "The specified duration is invalid (must be at least 60 seconds, and shorter than 28 days)."
            );
        ...
        try {
            ...
            return await interaction.editReply({
                content: `Failed to timeout user or log: ${err}`
            });
        }
    }
}
