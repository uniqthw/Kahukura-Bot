    ...
    slashCommand = new SlashCommandBuilder()
        ...
        .addIntegerOption((option) =>
            option
                .setName("amount")
                .setDescription(
                    "Specify the number of messages to bulk delete."
                )
                .setMinValue(1)
                .setMaxValue(100)
                .setRequired(true)
        ...
        .addStringOption((option) =>
            option
                .setName("reason")
                .setDescription(
                    "Please provide a justification for purging these messages."
                )
                .setRequired(true)
        )
        .addUserOption((option) =>
            option
                .setName("user")
                .setDescription(
                    "Specify if you would like to purge messages from a specific user."
                )
                .setRequired(false)
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<any> {
        ...
        if (!interaction.channel || !interaction.channel.isTextBased())
            return await interaction.editReply({
                content: "You can only purge messages in a text channel."
            });
        ...
        try {
            ...
            if (targetUser) {
                ...
                if (userMessages.size === 0)
                    return interaction.editReply(
                        "Unable to purge as no messages from that user were found in the last 100 messages in the channel."
                    );
            ...
            await interaction.editReply({
                content: "Failed to purge messages or log"
            });
