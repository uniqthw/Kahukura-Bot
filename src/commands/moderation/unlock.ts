import { Command } from "../../../@types";
import { ChatInputCommandInteraction, SlashCommandBuilder, GuildMember, TextChannel, PermissionFlagsBits } from "discord.js";
import { hasModeratorRole } from "../../utils/roleCheck";
import { logModAction } from "../../utils/modlog";
import settings from "../../../settings.json";

export default class UnlockCommand implements Command {
    name = "unlock";
    description = "Unlock the current channel (allow sending messages).";
    slashCommand = (new SlashCommandBuilder()
        .setName(this.name)
        .setDescription(this.description)
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels)
        .addStringOption(option =>
            option.setName("reason").setDescription("Reason for unlocking channel").setRequired(false)
        ) as SlashCommandBuilder);

    async execute(interaction: ChatInputCommandInteraction): Promise<any> {
        // Always defer reply for interaction timing
        await interaction.deferReply({ ephemeral: true });
        const member = interaction.member as GuildMember;
        
        // Permission check: must have moderator role
        if (!hasModeratorRole(member)) {
            return await interaction.editReply({ content: "You do not have permission to use this command." });
        }
        
        // Get command options
        const reason = interaction.options.getString("reason") || "No reason provided";
        const channel = interaction.channel as TextChannel;
        
        try {
            // Unlock channel by restoring send messages permission for @everyone
            await channel.permissionOverwrites.edit(
                channel.guild.roles.everyone, 
                { 
                    SendMessages: null // Reset to default
                },
                { reason: `Unlocked by ${member.user.tag}: ${reason}` }
            );
            
            // Log moderation action
            await logModAction({
                action: "unlock",
                targetId: channel.id,
                targetTag: `#${channel.name}`,
                moderatorId: member.user.id,
                moderatorTag: member.user.tag,
                reason,
                timestamp: Date.now(),
                guildId: interaction.guild?.id || ""
            });
            
            // Send public message about unlock
            await channel.send(`🔓 This channel has been unlocked by <@${member.user.id}>. Reason: ${reason}`);
            
            return await interaction.editReply({ 
                content: `Channel <#${channel.id}> has been unlocked. Reason: ${reason}` 
            });
        } catch (err) {
            // Error handling
            return await interaction.editReply({ content: `Failed to unlock channel: ${err}` });
        }
    }
}