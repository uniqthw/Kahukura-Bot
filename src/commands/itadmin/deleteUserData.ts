    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<any> {
        ...
        try {
            ...
            if (confirmation.customId === "confirm") {
                ...
                try {
                    await MongoDb.getInstance().deleteVerificationUserData(
                        user.id
                    );
                } catch (error) {
                    await confirmation.editReply({
                        content:
                            "An error occurred whilst deleting the user's data.",
                        components: []
                    });
                    return console.error(
                        "An error occurred whilst deleting a user's data:",
                        error
                    );
                }
                ...
                try {
                    ...
                    if (member && member.kickable)
                        await member.kick(
                            "User's data was deleted from the verification database"
                        );
                } catch (error) {
                    console.error(
                        "An error occurred whilst attempting to kick a user whose data has been deleted:",
                        error
                    );
                }

                await confirmation.editReply({
                    content: "Action confirmed. Data has been deleted.",
                    components: []
                });
            ...
            await interaction.editReply({
                content:
                    "Confirmation was not received within 1 minute, cancelling deletion.",
                components: []
            });
        }
    }
