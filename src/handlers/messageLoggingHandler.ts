...
export default class MessageLoggingHandler {
    async logPurgeAction(
        ...
        const textDisplayComponents = [
            new TextDisplayBuilder().setContent(
                `# ${MessageLogIcons.PURGE} Purge ${deletedCount} Messages`
            ),
            new TextDisplayBuilder().setContent(
            ...
            new TextDisplayBuilder().setContent(
                `**Executor:** ${userMention(moderator.id)} [\`${moderator.id}\`]`
            )
        ];
        ...
        if (targetUser) {
            textDisplayComponents.push(
                new TextDisplayBuilder().setContent(
                    `**Victim:** ${userMention(targetUser.id)} [${targetUser.id}]`
                )
            );
        }
        ...
        if (deletedMessages && deletedMessages.length > 0) {
            ...
            const sortedMessages = deletedMessages.sort(
            ...
            if (fileBuffer.byteLength === 0) {
                ...
                attachment = new AttachmentBuilder(fileBuffer, {
                    name: attachmentName
                });
        ...
        if (
            !messageLogChannel ||
            !messageLogChannel.isTextBased() ||
            !messageLogChannel.isSendable()
        ) {
            throw new Error(
                "Failed to message log to channel. Likely misconfiguration, ensure settings.json is appropriately filled out and the bot has access to send messages in the channel."
            );
        }
    ...
    async logMessageDelete(
        ...
        const textDisplayComponents = [
            ...
            new TextDisplayBuilder().setContent(
                `**Last edited at:** <t:${Math.floor((message.editedTimestamp || message.createdTimestamp) / 1000)}:F>`
            ),
    ...
    async logMessageUpdate(
        message: OmitPartialGroupDMChannel<
            Message<boolean> | PartialMessage<boolean>
        >,
        newMessage: OmitPartialGroupDMChannel<
            Message<boolean> | PartialMessage<boolean>
        >,
        client: Client
    ) {
